...
var verifyMatches = rootRequire('utils/verify-matches');
...
PrivateCliCmd.prototype.append = function(subStr, delimiter) {
	delimiter = delimiter || ' ';
	var newStr = [this.get(), subStr].join(delimiter);
	this.set(newStr);
	return this;
}
...
PrivateCliCmd.prototype.passAlong = function(userOps) {
	var passAlongs = userOps.getPassAlongs();

	var flags = _.pluck(passAlongs, 'flag');

	var result = ' -' + [flags].join('');
